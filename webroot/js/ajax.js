...
if( document.getElementById('communeIN')){
 ...
 if( document.getElementById('villeIN')){
	...
	var signaletique = document.getElementById('signaletique').value;
  	var codemodul = document.getElementById('codemodul').value;
...
var url2="https://"+window.location.host+"/siehm2017/folder/verifechamps.php"               
...
if(document.getElementById("Raison_sociale")){
			...
			if(donne1.codemodul){
				 document.getElementById("codemodul").style.border= "1px solid red";
				 			 if(donne1.codemodul!='ce champ est vide !'){
				 	   html=html+'<br/>'+donne1.codemodul
				 }
			}else{
if(document.getElementById("codemodul")){
				 document.getElementById("codemodul").style.border= "1px solid #ccc";
				 }
			}
			...
			var form=document.getElementById("dicbtt")
						...
						form.appendChild(etapesuivant);
							 etapesuivant.setAttribute("type",'hidden');
							  etapesuivant.setAttribute("name",'etape4');
							  etapesuivant.setAttribute("value",'VALIDER');
			document.forms['formep2'].submit();
			//this.form.submit()
		}
function print_enligne(key){
              var objxhr=xhr_connect()
              if (objxhr){
                  objxhr.onreadystatechange=function(){
                      if(objxhr.readyState==4){
                          if(objxhr.status==200){              
                         action_print_enligne( objxhr.responseText);
                        //	 alert( objxhr.responseText);
                          }
                      }
                  }   
                  //var args=arg.value;
          var donn="keyC="+key;    
          var url="/folder/cabinetdetail.php"  
          ...
          function action_print_enligne(donnee){
            var donne1=eval('('+donnee+')');
          //+' '+donne1.specialename.name
            var text1=donne1.civilite+' '+donne1.nom+' '+donne1.prenom+' '+donne1.organisme_visiteur+' '+donne1.adresse+' '+donne1.commune+' '+donne1.villename.name+' '+donne1.paysname.nam_pays



            var text = text1.replace(/^[\s\u3000]+|[\s\u3000]+$/g, '')
            var n=donne1.civilite+' '+donne1.nom+' '+donne1.prenom+'<br/>'+donne1.organisme_visiteur




            //update_qrcode2(text,n)
            //window.submit();
          }
