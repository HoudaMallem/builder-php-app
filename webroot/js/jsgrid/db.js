(function() {
        ...
        loadData: function(filter) {
            ...
            return $.grep(this.clients, function(client) {
                ...
                return (!filter.Code || client.Code.indexOf(filter.Code) > -1)
                    && (!filter.Produit || client.Produit === filter.Produit)
                    && (!filter.Marque || client.Marque.indexOf(filter.Marque) > -1)
                    && (!filter.Pays || client.Pays === filter.Pays) 
                   // && (!filter.Image || client.Image === filter.Image)
            });
        },
    ...
    window.db = db;
     db.clients =  []
                        $.ajax({
            type: "POST",
             url: "https://"+window.location.host+"/siehm2017/folder/fichecataloque.php",
            dataType: "text",
                success: function(data) {
                    if(data.length>1){
                        //alert(data);
                        var donne1=eval('('+data+')');

                        //alert(donne1.fiche);
                        db.clients =donne1.fiche;
  ...
  $.ajax({
            type: "GET",
            url: "https://"+window.location.host+"/siehm2017/folder/payslist.php",
            dataType: "text",
                success: function(data) {
                //   alert(data)
                var donne1=eval('('+data+')');
                var payss =donne1.pays
             for (k=0 ;k< payss.length ; k++){ 
                db.countries.push(payss[k]);
             }
           var codess =donne1.codes
             for (k=0 ;k<codess.length ; k++){ 
                 db.codes.push(codess[k]);
             }
